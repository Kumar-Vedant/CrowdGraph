...
function CommunityDashboard() {
    ...
    const data = communities.find(c => c.id === communityId);
          ...
          <div className="layout-content-container flex flex-col max-w-[920px] flex-1">
            <div className="flex flex-wrap justify-between gap-3 p-4">
              <div className="flex min-w-72 flex-col gap-3">
                <p className="text-[#110d1b] tracking-light text-[32px] font-bold leading-tight">{data.title}</p>
                <p className="text-[#5f4c9a] text-sm font-normal">Owner: {data.owner.name} · Created at: {data.createdAt.toLocaleDateString()}</p>
                <p className="text-[#5f4c9a] text-sm font-normal leading-normal">{data.members.length} members · Reputation: {data.reputation}</p>
              </div>
              <button
